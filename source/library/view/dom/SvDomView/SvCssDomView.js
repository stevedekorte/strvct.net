...
/**
 * @class SvCssDomView
 * @extends SvElementDomView
 * @description CSS related state and behavior.
 */

(class SvCssDomView extends SvElementDomView {
                ...
                if (doesSanityCheck) {
                    ...
                    if (!(key in ignoredKeys)) {
                        ...
                        if (resultValue != newValue) {
                            let msg = "SvDomView: style['" + key + "'] not set to expected value\n";
    ...
    */
    ...
    transitions () {
        if (this._transitions == null) {
            this._transitions = SvDomTransitions.clone().setDomView(this).syncFromDomView();
        }
    ...
    */
    ...
    calcSize () {
        ...
        const size = SvPoint.clone().setXY(w, h).freeze();
    ...
    cacheClientSize () {
        if (this.display() === "none") {
            return SvPoint.clone().freeze();
        }

        const e = this.element();
        this.setCachedSize(SvPoint.clone().setXY(e.clientWidth, e.clientHeight).freeze());
        return this;
    }
    ...
    */
    ...
    frameInViewport () {
        ...
        const frame = SvRectangle.clone().setOrigin(origin).setSize(size);
        return frame;
    }

    positionInViewport () {
        const box = this.boundingClientRect();
        return SvPoint.clone().set(Math.round(box.left), Math.round(box.top));
    }

    sizeInViewport () {
        const box = this.boundingClientRect();
        return SvPoint.clone().set(Math.round(box.width), Math.round(box.height));
    }
    ...
    frameInDocument () {
        ...
        //onst frame = SvRectangle.clone().setOrigin(origin)
    ...
    fixedFrame () {
        ...
        const origin = SvPoint.clone().set(Math.round(this.leftPx()), Math.round(this.topPx()));
        const size   = SvPoint.clone().set(Math.round(this.minWidthPx()), Math.round(this.minHeightPx()));
        const frame  = SvRectangle.clone().setOrigin(origin).setSize(size);
        return frame;
    }
    ...
    positionInDocument () {
        ...
        const p = SvPoint.clone().set(Math.round(left), Math.round(top));
        return p;
    }

    size () {
        return SvEventPoint.clone().set(this.clientWidth(), this.clientHeight());
    }
    ...
    */
    ...
    setContentAfterOrBeforeString (aString, afterOrBefore) {
        ...
        if (e.className.indexOf(uniqueClassName) === -1) {
            const newRuleKey = "SvDomView" + uniqueClassName + ":" + afterOrBefore;
