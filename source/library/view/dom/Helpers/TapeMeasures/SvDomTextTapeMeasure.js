...
/**
 * @class SvDomTextTapeMeasure
 * @extends ProtoClass
 * @classdesc Used to measure rendered text dimensions given a string and a style.
 *
 * Example uses:
 *
 *     const size1 = SvDomTextTapeMeasure.shared().sizeOfCssClassWithHtmlString(this.elementClassName(), text);
 *     const h = size1.height;
 *
 *     const size2 = SvDomTextTapeMeasure.shared().sizeOfElementWithHtmlString(domElement, text);
 *     const w = size2.width;
 *
 * TODO: move to using separate document
 */
(class SvDomTextTapeMeasure extends ProtoClass {

    /**
     * @description Initializes the prototype slots for the SvDomTextTapeMeasure class.
     */
    initPrototypeSlots () {
        {
            /**
     ...
     * @returns {SvDomTextTapeMeasure} The current instance.
     * @category Measurement
     */
    clean () {
        const e = this.testElement();
