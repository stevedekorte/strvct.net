...
(class SvWebBrowserScreen extends ProtoClass {
    ...
    isRotated () { // screen aspect doesn't match window (only works on mobile)
        ...
        const b = SvWebBrowserWindow.shared().aspectRatio() > 1;
        return a !== b && SvWebBrowserWindow.shared().isOnMobile();
    }
    ...
    lesserOrientedSize () {
        ...
        const w = Math.min(this.orientedWidth(), SvWebBrowserWindow.shared().width());
        const h = Math.min(this.orientedHeight(), SvWebBrowserWindow.shared().height());
