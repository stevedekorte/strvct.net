 ...
 * @description A class for recognizing gestures.
    ...
    This class:
     ...
     - coordinates which gestures are active with a SvGestureManager
    ...
    See SvSlideGestureRecognizer, for an example subclass.
    ...
    Simulating Touches with the SvMouse
    ...
    gesture, the SvGestureManager will detect this and let the child steal control.

*/

(class SvGestureRecognizer extends ProtoClass {

    initPrototypeSlots () {
        /**
         * @member {SvDomView} viewTarget - the view that the gesture is recognized on
         */
        {
         ...
         */
        {
            const slot = this.newSlot("moveListenerClasses", ["SvMouseMoveListener", "SvTouchMoveListener"]);
            slot.setSlotType("Array");
        }
         ...
         */
        {
            const slot = this.newSlot("defaultListenerClasses", ["SvMouseListener", "SvTouchListener"]);
            slot.setSlotType("Array");
        }
    ...
    init () {
        ...
        //this.setGestureName(this.svType().before("SvGestureRecognizer"))
     ...
     * @returns {SvGestureRecognizer} The updated gesture recognizer.
     */
    clearEvents () {
     ...
     * @returns {SvPoint} The current position.
     */
    currentPosition () {
     ...
     * @returns {SvPoint} The down position.
     */
    downPosition () {
     ...
     * @returns {SvPoint} The begin position.
     */
    beginPosition () {
     ...
     * @returns {SvPoint} The up position.
     */
    upPosition () {
    ...
    requestActivationIfNeeded () {
        if (this.shouldRequestActivation()) {
            return SvGestureManager.shared().requestActiveGesture(this);
        }
    ...
    isActive () {
        if (this.shouldRequestActivation()) {
            return SvGestureManager.shared().activeGesture() === this;
        }
    ...
    deactivate () {
        if (this.shouldRequestActivation()) {
            SvGestureManager.shared().deactivateGesture(this);
        }
    ...
    didFinish () {
        ...
        SvGestureManager.shared().removeBegunGesture(this);
    ...
    pointsForEvent (event) {
        ...
        if (eventClass === MouseEvent) {
            ...
            return SvMouse.shared().pointsForEvent(event);
        } else if (eventClass === TouchEvent) {
            //this.logDebug(" got touch")
            return SvTouchScreen.shared().pointsForEvent(event);
        }
     ...
     * @returns {SvPoint} The difference between the current position and the begin position.
     */
    diffPos () {
    ...
    sendBeginMessage () {
        ...
        SvGestureManager.shared().addBegunGesture(this);
        return this;
    }
     ...
     * @param {SvGestureRecognizer} byGesture - The gesture that requests the cancellation.
     * @returns {SvGestureRecognizer} The updated gesture recognizer.
     */
    ...
    */
     ...
     * @returns {SvDomView} The new outline view.
     */
    newOutlineView () {
     ...
     * @returns {SvDomView} The outline view.
     */
    outlineView () {
    ...
    showOutlineView () {
        const v = this.outlineView();
        if (!v.parentView()) {
            SvDocumentBody.shared().addSubview(v);
        }
        const vt = this.viewTarget();
     ...
     * @returns {SvDomView} The new finger view.
     */
    newFingerView () {
        const v = SvDomView.clone();
        v.setPointerEvents("none");
     ...
     * @returns {SvDomView} The view for the finger ID.
     */
    viewForFingerId (id) {
    ...
    updateDebugTimer () {
        ...
        this.addWeakTimeout(() => this.updateDebugger(), ms, this.updateDebuggerTimeoutName());
        return this;
    }
     ...
     */
    allPoints () { // TODO: some better abstraction for Touch+SvMouse?
        const points = [];
        points.appendItems(SvTouchScreen.shared().currentPoints());
        points.appendItems(SvMouse.shared().currentPoints());
        return points;
    }
     ...
     */
    allDownPoints () { // TODO: some better abstraction for Touch+SvMouse?
     ...
     */
    shortTypeId () {
        return this.svTypeId().replaceAll("SvGestureRecognizer", "");
    }
