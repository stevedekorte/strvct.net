...
/**
 * @class SvEventListener
 * @extends ProtoClass
 * @classdesc Listener for single event. Should only be used by SvEventSetListener class.
 *
...
(class SvEventListener extends ProtoClass {
    ...
    static showActive () { // tmp debugging
        ...
        console.log("--- SvEventListener " + owners.size + " active owners ---");
        /*
    ...
    /**
     * @description Initializes the SvEventListener.
     * @returns {SvEventListener} The initialized SvEventListener.
     */
    init () {
        super.init();
     ...
     * @returns {SvEventListener} The SvEventListener instance.
     */
    setListenTarget (t) {
        if (this.isListening()) {
            assert(t);
    ...
    incrementListenCount () {
        ...
        //SvEventListener.activeListeners().add(this)
    ...
    decrementListenCount () {
        this.thisClass().listenCount--;
        //SvEventListener.activeListeners().delete(this)
    ...
    ownerDescription () {
        ...
        if (d.viewTarget) {
            //return d.viewTarget().svDebugId() + "->" + d.svType().before("SvGestureRecognizer")
            return d.viewTarget().svTypeId() + " ->" + d.svType().before("SvGestureRecognizer");
        }
    ...
    safeHandleEvent (event) {
        const result =  SvEventManager.shared().safeWrapEvent(() => {
    ...
    onAfterEvent (/*event*/) {
        if (this.isUserInteraction()) {
            SvEventManager.shared().onReceivedUserEvent();
        }
        return this;
    }
