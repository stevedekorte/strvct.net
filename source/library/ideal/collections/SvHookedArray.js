 ...
 * @class SvHookedArray
 * @extends Array
 * @classdesc A subclass of Array that hooks the base mutation methods so we can track mutations to the array and call willMutate and didMutate hooks. For this to work, you need to use method alternatives to the non-method array operations:   - a[i] -> instead use a.at(i)   - a[i] = b -> instead use a.atPut(i, b)   - delete a[i] -> instead use a.removeAt(i) To do this without using method alternatives, we would need to use a Proxy to intercept the array operations, which would be safer, but also slower and more memory intensive.
 */
(class SvHookedArray extends Array {

    /**
     * Returns Array as the species for derived arrays.
     * This ensures that methods like map(), filter(), slice() return plain Arrays
     * instead of SvHookedArray subclass instances, avoiding complex initialization
    ...
    /**
     * @returns {SvHookedArray} A read-only shallow copy of the array
     * @category Array Operations
     */
    ...
    /**
     * Performs a self-test of the SvHookedArray class
     * @returns {Function} The SvHookedArray class
     * @category Testing
     */
    static selfTest () {
        const a = this.clone();
