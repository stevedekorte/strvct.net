 ...
 *   }
 */
(class SvGeminiRequest extends SvAiRequest {

    initPrototypeSlots () {
    /**
     * @member {SvJsonStreamReader} jsonStreamReader - The JSON stream reader for handling streamed responses.
     * @category Data Processing
     */
        {
            const slot = this.newSlot("jsonStreamReader", null);
            slot.setSlotType("SvJsonStreamReader");
    ...
    /**
   * @description Initializes the SvGeminiRequest instance.
   ...
   */
    init () {
        ...
        const reader = SvJsonStreamReader.clone();
   ...
   */
    async apiKey () {
        return await SvGeminiService.shared().apiKeyOrUserAuthToken();
    }
   ...
   */
    async getApiUrl () {
        return await SvGeminiService.shared().getChatEndpointWithKey();
    }

    /**
   * @description Sets up the request for streaming.
   * @returns {SvGeminiRequest} The current instance.
   ...
   */
    ...
    async isUsingUserAuthToken () {
        return await SvGeminiService.shared().isUsingUserAuthToken();
    }
   ...
   */
    async requestOptions () {
        ...
        const token = await SvGeminiService.shared().apiKeyOrUserAuthToken();
        if (token) {
