...
(class SvFirestoreDocuments extends SvSummaryNode {
    ...
    initPrototype () {
        ...
        this.setSubnodeClasses([SvFirestoreDocument]);
    ...
    async asyncRefresh () {
        ...
        if (!parent || !parent.path) {
            throw new Error("SvFirestoreDocuments must have a SvFirestoreCollection parent to refresh");
        }
    ...
    documentWithDocIdCreateIfAbsent (docId) {
        ...
        if (!doc) {
            ...
            if (parent && parent.isKindOf(SvFirestoreQuery)) {
                collection = parent.ownerNode();
            }

            if (collection && collection.isKindOf(SvFirestoreCollection)) {
    ...
    async asyncAtPutDocument (storagePath, jsonContent) {
        ...
        if (doc) {
            ...
            const newDoc = SvFirestoreDocument.clone();
    ...
    refreshActionInfo () {
        ...
        const hasParent = parent && parent.isKindOf(SvFirestoreCollection);
