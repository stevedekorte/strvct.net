"use strict";

/**
 * @module library.services.Firebase
 */

/**
 * @class SvFirebaseService
 * @extends SvAiService
 * @classdesc Root service for Firebase integration
 */
(class SvFirebaseService extends SvSummaryNode {

    /**
     * @static
     * @description Initializes the class and sets it as a singleton.
     * @category Initialization
     */
    static initClass () {
        this.setIsSingleton(true);
    }

    initPrototypeSlots () {

        {
            const slot = this.newSlot("firestoreDatabaseService", null);
            slot.setFinalInitProto(SvFirestoreDatabaseService);
            slot.setIsSubnodeField(true);
            slot.setShouldStoreSlot(false);
            slot.setSyncsToView(true);
        }

        {
            const slot = this.newSlot("firebaseStorageService", null);
            slot.setFinalInitProto(SvFirebaseStorageService);
            slot.setIsSubnodeField(true);
            slot.setShouldStoreSlot(false);
            slot.setSyncsToView(true);
        }

    }

    initPrototype () {
        this.setTitle("Firebase");
        this.setSubtitle("could services");
        this.setShouldStoreSubnodes(false);
        this.setShouldStore(true);
    }

    finalInit () {
        super.finalInit();
        this.initPrototype();
    }

    addSubnode (subnode) {
        if (this.subnodes().length > 2) {
            debugger;
        }
        super.addSubnode(subnode);
    }

}.initThisClass());
