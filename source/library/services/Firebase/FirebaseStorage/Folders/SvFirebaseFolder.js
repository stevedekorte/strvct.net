...
/**
 * @class SvFirebaseFolder
 * @extends SvFirebaseNode
 ...
 * a mix of SvFirebaseFolder and SvFirebaseFile instances, creating a hierarchical
 ...
 */
(class SvFirebaseFolder extends SvFirebaseNode {
    ...
    initPrototype () {
        ...
        this.setSubnodeClasses([SvFirebaseFolder, SvFirebaseFile]);
     ...
     * @returns {Array<SvFirebaseFolder>} Array of subfolders
     * @category Query
     */
    subfolders () {
        return this.subnodes().filter(node => node.svType() === "SvFirebaseFolder");
    }

    /**
     * @description Gets all files in this folder
     * @returns {Array<SvFirebaseFile>} Array of files
     * @category Query
     */
    files () {
        return this.subnodes().filter(node => node.svType() === "SvFirebaseFile");
    }
     ...
     * @returns {SvFirebaseFolder} The found or created folder
     * @category Helper
     */
    subfolderNamedCreateIfAbsent (name) {
        let subfolder = this.subfolderNamed(name);
        if (!subfolder) {
            subfolder = SvFirebaseFolder.clone();
        ...
        const promise = (async () => {
            try {
                ...
                for (const subnode of existingSubnodes) {
                    ...
                    if (!subnode.isKindOf(SvFirebaseNode)) {
                        throw new Error("subnode is not a SvFirebaseNode: " + subnode.svType());
                    }

                    const name = subnode.name();
                    if (subnode.svType() === "SvFirebaseFolder") {
                        if (!serverFolderNames.has(name)) {
                            this.removeSubnode(subnode);
                        }
                    } else if (subnode.svType() === "SvFirebaseFile") {
                ...
                for (const prefixRef of result.prefixes) {
                    ...
                    if (!existingFolder) {
                        const subfolder = SvFirebaseFolder.clone();
                ...
                const newFiles = await Promise.all(
                    result.items
                        ...
                        .map(itemRef => SvFirebaseFile.clone().setItemRef(itemRef))
                );
     ...
     * @param {string} name - The folder name
     * @returns {SvFirebaseFolder} The created folder instance
     * @category Storage Operations
     */
