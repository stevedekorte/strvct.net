...
(class SvToolCall extends SvJsonDictionaryNode {
    ...
    // only the SvToolCall class should know what it's schema is
    //const schema = this.asRootJsonSchema(new Set()).deepCopy();
        ...
        schema["$id"] = toolName + "_SvToolCall";
   ...
   */
    initPrototypeSlots () {
...

        {
            ...
            slot.setDescription("Reference to the owner SvToolCalls instance.");
            slot.setSlotType("SvToolCalls");
...

        {
            ...
            slot.setSlotType("SvToolDefinition");
...

        {
            ...
            slot.setSlotType("SvAiResponseMessage");
            slot.setIsInJsonSchema(false);
            slot.setIsSubnodeField(false);
            slot.setCanEditInspection(false);
            slot.setShouldStoreSlot(true);
        ...
        /*
    {
        ...
        */
...

        {
            ...
            slot.setDescription("A SvToolResult instance.");
            slot.setSlotType("SvToolResult"); // subclasses should override this a needed
            slot.setShouldJsonArchive(false);
            slot.setIsSubnodeField(true);
            slot.setCanEditInspection(false);
            slot.setIsInJsonSchema(false);
    ...
    parseCallString () {
        try {
            ...
            if (!callString.isValidJson()) {
                console.error("Error parsing tool call. Attempting to fix with SvBasicJsonRepairShop.");

                const repairShop = new SvBasicJsonRepairShop();
    ...
    descriptionForSchemaValidationError (errorString) {
        const callSchema = SvToolCall.toolCallJsonSchemaForToolDefinition(this.toolDefinition());
    ...
    assertValidToolCallSchema () {
        ...
        const callSchema = SvToolCall.toolCallJsonSchemaForToolDefinition(this.toolDefinition(), refSet);
        ...
        // See if our own SvToolCall schema is valid
        ...
        // Now see if the call JSON is is valid against our own SvToolCall schema
 ...
 */
    ...
    //throw new Error(this.svType() + " Error parsing call string: " + e.message);
        ...
        // lets get the tool definition schema so we can remind the AI of the expected format
            ...
            if (toolDefinition !== null) {
                const toolCallSchema = SvToolCall.toolCallJsonSchemaForToolDefinition(toolDefinition);
    ...
    shutdown () {
        ...
        this.setStatus("completed");
        this.setToolResult("aborted by call to SvToolCall.shutdown()");
        return this;
    }
