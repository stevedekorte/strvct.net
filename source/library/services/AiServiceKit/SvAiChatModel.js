 ...
 */
(class SvAiChatModel extends SvSummaryNode {
    /**
   * Initializes the prototype slots for the SvAiChatModel.
   * @category Initialization
   */
    initPrototypeSlots () {
    /**
     * @member {SvAiService} service - The AI service associated with this model.
     * @category Service
     */
        {
    ...
    /**
   * Performs final initialization of the SvAiChatModel.
   * @category Initialization
   */
    finalInit () {
        super.finalInit();
   ...
   * @returns {SvAiService|Object} The service or parent node's parent node.
   * @category Service
   */
    service () {
        if (this._service) {
   ...
   * @returns {SvAiChatModel} The current instance.
   * @category Model Configuration
   */
    setJson (json) {
   ...
   */
    summary () {
        const cw = SvNumberFormatter.clone().setValue(this.inputTokenLimit()).setSignificantDigits(2).formattedValue();
