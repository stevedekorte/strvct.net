...
(class SvAiRequest extends SvStorableNode {
     ...
     */
        ...
        /**
     * @member {SvAiChatModel} model - The model the request is for.
     */
        {
   ...
   */
    ...
    // NOTE: it's not idea for the SvAiRequest to know about the SvConversationMessage or SvConversation class
    // TODO: refactor this later so the prepareToSendRequest() method somehow get the messaage reference
    message () {
        const delegate = this.delegate();
        if (delegate && delegate.isKindOf(SvConversationMessage)) {
            return delegate;
        }
        return null;
    }
   ...
   */
    proxyUrl () {
        const proxyUrl = SvProxyServers.shared().defaultServer().proxyUrlForUrl(this.url());
        return proxyUrl;
    }
   ...
   */
    ...
    // For streaming, use fullContent if available, otherwise get from current XHR
        if (this.fullContent()) {
            return SvByteFormatter.clone().setValue(this.fullContent().length).formattedValue();
        }
        const xhr = this.currentXhrRequest();
        if (xhr) {
            return SvByteFormatter.clone().setValue(xhr.contentByteCount()).formattedValue();
        }
        return SvByteFormatter.clone().setValue(0).formattedValue();
    }
   ...
   */
    retryRequest () {
        ...
        this.setReadIndex(0); // this is the read index on the new xhr responseText, not the SvAiRequest fullContent
