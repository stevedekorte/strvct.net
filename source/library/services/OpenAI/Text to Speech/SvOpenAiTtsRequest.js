...
/**
 * @class SvOpenAiTtsRequest
 * @extends SvSummaryNode
 * @classdesc Represents a Text-to-Speech request to the OpenAI API.
 */
(class SvOpenAiTtsRequest extends SvSummaryNode {

    /**
   * @description Initializes the prototype slots for the SvOpenAiTtsRequest class.
   */
    initPrototypeSlots () {
    /**
    ...
    /**
   * @description Initializes the SvOpenAiTtsRequest instance.
   ...
   * @returns {SvOpenAiService} The shared OpenAI service instance.
   * @category Service
   */
    service () {
        return SvOpenAiService.shared();
    }
   ...
   * @returns {SvOpenAiTtsRequest} The current instance.
   * @category Configuration
   */
    setService (anObject) {
   ...
   */
    proxyUrl () {
        const SvProxyServers = SvGlobals.get("SvProxyServers");
        if (SvProxyServers && SvProxyServers.shared().defaultServer()) {
            return SvProxyServers.shared().defaultServer().proxyUrlForUrl(this.apiUrl());
        }
        return this.apiUrl();
    }
        ...
        try {
            ...
            if (!xhrRequest.isSuccess()) {
                ...
                if (statusCode === 0) {
                    ...
                    // Resolve (not reject) to avoid unhandled rejection in SvAudioQueue
            ...
            if (this.isDebugging()) {
                console.log("SvOpenAiTtsRequest: Request completed\n" + xhrRequest.description());
            }
            ...
            try {
                ...
                if (this.isDebugging()) {
                    console.log("SvOpenAiTtsRequest: Loading audio blob into SvWaSound...");
                }
                await sound.asyncLoadFromDataBlob(audioBlob);
                if (this.isDebugging()) {
                    console.log("SvOpenAiTtsRequest: Audio loaded", JSON.stringify({
                        data: sound.data(),
