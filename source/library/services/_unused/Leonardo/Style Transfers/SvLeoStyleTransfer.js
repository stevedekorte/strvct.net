...
/**
 * @class SvLeoStyleTransfer
 * @extends SvSummaryNode
 * @classdesc How it works:
 * - takes a prompt
 * - uses SvOpenAiImagePrompt to generate an image
 * - uses SvLeonardoImagePrompt to perform a style transfer on the image
 * - stores the style transferred image as a urlData
 * - disposes of the SvOpenAiImagePrompt and the SvLeonardoImagePrompt objects
 *
 *
...
(class SvLeoStyleTransfer extends SvSummaryNode {
    /**
   * @description Initializes the prototype slots for the SvLeoStyleTransfer class.
   */
    initPrototypeSlots () {
        ...
        // OpenAI image generation prompt
        {
            ...
            slot.setFinalInitProto(SvOpenAiImagePrompt);
            slot.setIsSubnodeField(true);
            slot.setShouldStoreSlot(true);
        }
        ...
        // Leonardo style transfer prompt
        {
            ...
            slot.setFinalInitProto(SvLeonardoImagePrompt);
            slot.setIsSubnodeField(true);
            slot.setShouldStoreSlot(true);
        }
        ...
        // Reference image for init
        {
            ...
            slot.setFinalInitProto(SvLeonardoRefImage);
            slot.setIsSubnodeField(true);
            slot.setShouldStoreSlot(true);
        }
   ...
   */
    ...
    setupLeonardoPrompt () {
        ...
        const modelId = SvLeonardoImagePrompt.modelIdForName("Phoenix 1.0");
    ...
    async selectStyleImage () {
        try {
            ...
            if (!this.styleRefImage()) {
                this.setStyleRefImage(SvLeonardoRefImage.clone());
            }
