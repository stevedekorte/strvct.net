...
- we create a SvLeonardoImageGeneration to poll for the status of the generation

SvLeonardoImageGeneration:
- polls for the status of the generation
- when the generation is complete, we get the image urls
- we create SvLeonardoImage nodes and add set their urls
...

{
  ...
  Next, create a SvLeonardoImageGeneration node with the generation id
  ...
  Next, we create SvLeonardoImage nodes, set their urls, and fetch them.


  */
(class SvLeonardoImagePrompt extends SvSummaryNode {
    ...
    static validRefImageItems () {
        return SvLeonardoService.shared().refImages().validItems();
    }
     ...
     */
        {
            const validItems = SvLeonardoImagePrompt.validModelIdItems();
        ...
        {
            const validItems = SvLeonardoImagePrompt.validPresetStyleItems();
   ...
   */
     ...
     */
        ...
        /**
     * @member {SvLeonardoImageGeneration} generation
     * @description The generation node.
     * @category Output
     */
        {
            const slot = this.newSlot("generation", null);
            slot.setFinalInitProto(SvLeonardoImageGeneration);
            slot.setShouldStoreSlot(true);
            slot.setIsSubnode(true);
            slot.setSlotType("SvLeonardoImageGeneration");
        }
   ...
   */
    service () {
        return SvLeonardoService.shared();
    }
   ...
   */
    ...
    async setupXhrRequest () {
        ...
        const proxyEndpoint = SvProxyServers.shared().defaultServer().proxyUrlForUrl(endpoint);
  ...
  */
   ...
   */
    ...
    // -- delegate methods from SvLeonardoImage --
   ...
   * @returns {SvOpenAiImagePrompt} The current instance.
   * @category Lifecycle
   */
    shutdown () {
        if (this.xhrRequest()) {
