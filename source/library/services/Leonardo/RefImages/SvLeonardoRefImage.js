...
(class SvLeonardoRefImage extends SvSummaryNode {
    /**
   * @description Initializes the prototype slots for the SvLeonardoRefImage class.
   */
    initPrototypeSlots () {
 ...
 */
   ...
   */
    getProxyUrl () {
        return SvProxyServers.shared().defaultServer().proxyUrlForUrl(this.url());
    }
   ...
   */
    ...
    async proxyXhrForUrl (url, method, bodyString) {
        const proxyUrl = SvProxyServers.shared().defaultServer().proxyUrlForUrl(url);
