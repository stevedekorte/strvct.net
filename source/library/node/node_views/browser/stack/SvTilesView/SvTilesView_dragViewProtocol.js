...
/** * @class SvTilesView_dragViewProtocol
 * @extends SvTilesView
 * @classdesc Handles the drag and drop protocol for SvTilesView.
 *
 * When a drag is performed, a SvDragView is created to render the drag operation,
 *  manage the drag events, and store the drag related info such as the list of items being dragged.
 * The SvDragView instance is used as an argument for many of the drag view protocol methods.
 *
 ...
 *
 * Note that we do the drop on the SvTilesView, not on the tile so we can manage
...
(class SvTilesView_dragViewProtocol extends SvTilesView {
     ...
     * @returns {SvTilesView_dragViewProtocol} The current instance
     */
    onDragSourceBegin (dragView) {
        this.setHasPausedSync(true);
        //SvElementDomView.pauseRetires();
    ...
    onDragSourceEnd (/*aDragView*/) {
        this.endDropMode();
        //SvElementDomView.unpauseRetires();
    }

    // -- messages sent by SvDragView to the potential drop view, if not the source ---
    ...
    newTilePlaceHolder (dragView) {
        ...
        if (!this.tilePlaceHolder()) {
            ...
            ph.setBackgroundColor("var(--sv-surface)");
