...
/**
 * @class SvSubObjectPool
 * @extends SvObjectPool
 * @description
 *
 *      A specialized SvObjectPool for cloud sync of session data.
 *      Uses a regular SvAtomicMap and serializes the complete object graph.
 *      Validates that certain classes (like UoApp, SvPersistentObjectPool) are not serialized.
 *
 *      Key features:
...
(class SvSubObjectPool extends SvObjectPool {
    ...
    /**
     * @description Initialize the SvSubObjectPool
     * @returns {SvSubObjectPool}
     */
    init () {
        super.init();
     ...
     * SvSubObjectPool uses manual save via asyncSaveToCloud instead of automatic scheduling.
     * @returns {SvSubObjectPool}
     */
    scheduleStore () {
        // No-op - SvSubObjectPool does not auto-schedule stores
    ...
    jsonStringForObject (obj) {
        ...
        if (this.isBannedClass(typeName)) {
            throw new Error("SvSubObjectPool: Cannot serialize banned class '" + typeName + "'");
        }
    ...
    collectDelta () {
        ...
        if (!snapshot) {
            this.isDebugging() && console.log("CLOUDSYNC [SvSubObjectPool] No snapshot - full upload needed");
            return null;
        }
        ...
        if (totalChanges === 0) {
            this.isDebugging() && console.log("CLOUDSYNC [SvSubObjectPool] No changes detected - skipping upload");
        ...
        if (totalRecords > 0 && totalChanges / totalRecords > this.thisClass().fullUploadThreshold()) {
            this.isDebugging() && console.log("CLOUDSYNC [SvSubObjectPool] Changes (" + totalChanges + "/" + totalRecords + ") exceed threshold - full upload");
            return null;
        }

        this.isDebugging() && console.log("CLOUDSYNC [SvSubObjectPool] Delta collected: " + Object.keys(writes).length + " writes, " + deletes.length + " deletes");
    ...
    async asyncSaveToCloud () {
        ...
        if (delta === null) {
            ...
            this.isDebugging() && console.log("CLOUDSYNC [SvSubObjectPool] Performing full upload for session:", sessionId);
            ...
            // Delta upload
            this.isDebugging() && console.log("CLOUDSYNC [SvSubObjectPool] Performing delta upload for session:", sessionId);
            ...
            if (deltaCount >= this.thisClass().compactionThreshold()) {
                this.isDebugging() && console.log("CLOUDSYNC [SvSubObjectPool] Delta count (" + deltaCount + ") exceeds threshold, compacting...");
    ...
    async asyncCompactToCloud () {
        ...
        this.isDebugging() && console.log("CLOUDSYNC [SvSubObjectPool] Compaction complete for session:", sessionId);
    }
    ...
    async asyncAcquireOrRefreshLock () {
        ...
        try {
            ...
            if (result.success) {
                ...
                this.isDebugging() && console.log("[SvSubObjectPool] Lock acquired for session:", sessionId);
                return true;
            }
            console.error("SvSubObjectPool: Lock acquisition failed:", result.error || result);
            return false;
        } catch (error) {
            console.error("SvSubObjectPool: Failed to acquire lock:", error);
            return false;
        }
    }
    ...
    async asyncReleaseLock () {
        ...
        try {
            ...
            console.error("SvSubObjectPool: Failed to release lock:", error);
            return false;
        }
    }
    ...
    startLockRefreshTimer () {
        ...
        this._lockRefreshTimerId = setInterval(() => {
            if (this.hasLock() && this.lockNeedsRefresh()) {
                this.isDebugging() && console.log("[SvSubObjectPool] Auto-refreshing lock for session:", this.sessionId());
                this.asyncAcquireOrRefreshLock().catch(error => {
                    console.error("SvSubObjectPool: Lock auto-refresh failed:", error);
                });
            }
