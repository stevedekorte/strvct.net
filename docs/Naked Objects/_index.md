...
The same annotations that drive UI generation also make the domain model legible and operable to AI agents. JSON Schema descriptions of any object are derived automatically from slot metadata, and AI services retrieve them via a tool call — on demand — to include in system prompts. A complementary tool call applies JSON Patch operations to live objects, so the agent reads the shape of any part of the graph and writes valid changes back, with the same structural guarantees (required fields, type constraints, valid references) that are enforced on human users.

This extends the naked objects premise. The original pattern exposed domain objects directly to users, eliminating the translation layer between model and UI. The same annotations now eliminate the translation layer between model and AI. No per-feature tool definitions are needed: a single pair of tools — "get schema for object X" and "apply patch to object X" — lets an agent inspect and modify any part of the domain graph. In Strvct's production use, an AI game master modifies session state, character sheets, and campaign data through exactly this loop, with no feature-specific AI integration code.

When a patch fails — invalid path, type mismatch, constraint violation, unresolvable reference — the error response identifies the exact failing path and includes the schema of the affected slot, so the agent can correct the patch on the next attempt without refetching context. This closes the loop: the same uniform structure and metadata that drive UI generation, persistence, and validation also drive the feedback that lets an agent self-correct. No separate AI-error vocabulary needs to be maintained, and an agent that understands how to read a slot annotation automatically understands how to read a rejection.

In a conventional framework, integrating an AI agent requires custom tool definitions for each editable surface, hand-maintained schema documents to describe what the AI may touch, and bespoke error-reporting paths for each. All three scale linearly with the domain model. In Strvct, one schema tool, one patch tool, and one error format cover the entire model, and the cost of exposing a new object to AI is zero.

### 7.2 Automatic Responsive Design

Because layout flows from node annotations — orientation, whether a tile fills remaining space, whether a collection displays inline or navigates to a detail view — rather than from screen-specific CSS, the interface adapts to viewport size without per-view breakpoints or media queries. The master-detail chain described in Section 4 illustrates this directly: earlier columns automatically collapse when the viewport narrows, a breadcrumb bar preserves the navigation path, and the same composition scales from a wide desktop monitor to a narrow mobile screen with no per-screen responsive code.

This is structurally the same mechanism as the other emergent capabilities: the framework owns the single point where model structure becomes UI, so responsive behavior can be expressed once at that seam and amortized across every screen. In a conventional framework, each screen declares its own layout and its own breakpoints; the cost of maintaining responsive behavior grows linearly with the number of screens. In Strvct, it is paid once at the primitive level.

### 7.3 Automatic Accessibility

Because every interactive surface is a tile generated from a small set of composable primitives rather than a bespoke component, accessibility can be built into the primitives and apply uniformly to every screen. Focus order, arrow-key traversal between sibling tiles, Enter to drill into a detail column, and Escape to return are properties of the tile and tile stack — not of individual views. An internal debugging interface is as keyboard-navigable as a character sheet, because both are composed from the same primitives.

The same structural knowledge that drives the rendered view also drives ARIA output. Each view class provides a default ARIA role derived from its function: tile stacks become `list` containers, individual tiles become `link` items, field tiles become `group` elements, boolean fields become `checkbox`, action fields become `button`. Slot metadata that already exists for type checking and schema generation — `isRequired()`, `isReadOnly()`, minimum/maximum constraints, `description()` — maps directly to ARIA attributes: `aria-required`, `aria-readonly`, `aria-valuemin`/`aria-valuemax`, `aria-description`. The node hierarchy provides landmarks (`navigation`, `region`) and breadcrumb structure. Dynamic content nodes can opt into `aria-live` announcements.

All three UI-surface capabilities in this section — responsive design, accessibility, and internationalization — exist because UI-visible information flows from annotated model slots through a single framework-controlled pipeline. In a conventional framework, accessibility is a per-component obligation — every button, list, and form field needs manual ARIA attributes and keyboard handlers, and the cost scales linearly with the number of components. In Strvct, fixing accessibility in the ~15 core view classes fixes it for every application built on the framework.
