...
Strvct implements the first operation through recursively composed, orientation-flexible master-detail views generated from the object graph. Tiles and tile stacks provide a uniform summary, editing, and navigation surface; Miller-column-style recursion preserves context at arbitrary depth and collapses automatically on narrow viewports. They already cover recursive hierarchy, ordered lists, and feed-like rich-item sequences. The remaining projection hypothesis is smaller than a catalog of named widgets: matrix mappings, categorical grouping, and continuous-axis mappings may account for tables, kanban boards, timelines, calendars, and related forms through declarative combinations of arrangement, semantic slot mappings, item presentation, and interaction rules.
...
3. A **semantic collection-projection hypothesis** in which familiar forms reduce to a smaller algebra. Existing tile stacks already supply linear collections and feed-like rich-item sequences, while recursive navigation supplies hierarchy; the principal missing families appear to be matrix, categorical-grouping, and continuous-axis mappings.
...
6. A working framework and production application demonstrating the feasibility of the implemented half of this grammar, while identifying broader collection projections as a testable direction rather than a completed result.
...
The original candidate list—list, grid, table, tree, feed, kanban, and timeline—overcounts the underlying ideas. Several forms collapse into behavior Strvct already has or into layout variants of it:
...
- **Table** is a matrix in which rows are objects, columns are selected slots, and each cell is a slot value.

This leaves three principal missing projection families, plus one open boundary:

| Family | Semantic mapping | Familiar forms |
| --- | --- | --- |
| Matrix | two meaningful axes → cell value | table, spreadsheet, pivot |
| Categorical grouping | property value → discrete group | kanban, swimlane, grouped list |
| Continuous axis | numeric or temporal value → position or extent | timeline, Gantt, calendar |
| Quantitative visualization (open) | values → graphical encodings | chart, plot, heatmap |

A matrix shares geometry across several products without erasing their interaction differences. A table maps objects × slots. A spreadsheet adds addressable cells, ranges, formulas, fill operations, and recalculation. A pivot maps grouped domain dimensions onto both axes. These may share a projection family while retaining specialized behavior.

The candidate algebra can be described through orthogonal choices:

- **Arrangement:** linear, wrapped, matrix, categorical columns, or continuous axis
...
Under that account, familiar UI names are presets rather than framework primitives. A feed is linear arrangement plus temporal or ranked ordering plus rich tiles. A kanban board is categorical grouping plus a rule that dragging changes the grouping slot. A timeline is a continuous time axis plus start and end slot mappings.
...
Strvct's UI approach has two layers. The first is implemented today: tiles, tile stacks, and recursive master-detail views derive navigation, inspection, ordered collections, and feed-like rich-item sequences from the model graph. The second is a disciplined direction for expansion: matrix, categorical-grouping, and continuous-axis projections derived from model semantics. Keeping their status distinct matters; the production case study validates the first, not the complete proposed projection algebra.
...
### 4.5 Collection Projections

The current tile stack is already the default linear projection. Ordering it temporally or by rank and giving each item a rich tile produces a feed without requiring a separate `FeedView`. Recursive hierarchy is likewise already present in master-detail navigation; an inline tree would be an alternate rendering of that topology. Wrapped tile layout may cover gallery-style grids without introducing new domain semantics.

A generalized collection-presentation layer could preserve the same model/view boundary while adding the projection families that remain. Conceptually:
...
describes a kanban projection, while:
...
describes a timeline. These examples illustrate the direction, not a current Strvct API.

A matrix declaration would map objects and slots onto axes:
...
This could produce a table while leaving spreadsheet selection, formulas, and fill behavior to richer matrix interactions. The point of the algebra is not to deny meaningful differences, but to share the semantic mappings and geometry beneath them.

The framework—not the model—must remain responsible for viewport decisions, gesture mechanics, and concrete rendering. The model may declare semantic hints such as grouping, ordering, or relevant slots; the view layer chooses how those hints become pixels on a particular platform. This preserves the naked-objects boundary and keeps the same application runnable under web, terminal, or headless user interfaces.

### 4.6 Companion Views: Preserving Secondary Context
...
*Projection maturity.* Strvct's recursive master-detail grammar is implemented and production-tested. The broader collection-projection algebra described in §3 and §4.5 is a proposal. Until those projections exist as framework-level behavior, the coverage gains argued here remain a hypothesis rather than a result.
...
*Build the missing projection families.* Matrix, categorical-grouping, and continuous-axis mappings should be explored over the same collection and slot metadata. Linear lists, feed-like rich tile sequences, and recursive hierarchy already exist; wrapped galleries may be a tile-layout variant. The open question is whether charts fit a generalized axis-and-encoding system or require a separate quantitative-visualization family.
...
The stronger grammar has two operations: traverse relationships through an object graph, and project collections along semantic dimensions. Strvct already implements recursive hierarchy, linear tile collections, and feed-like rich-item sequences through tiles, tile stacks, and master-detail navigation. The principal missing families now appear smaller: matrix, categorical grouping, and continuous-axis mapping, with quantitative visualization as an open boundary. Tables, spreadsheets, pivots, kanban boards, timelines, and calendars are not necessarily peer widgets; they may be configurations of recurring spatial and semantic mappings attached to the model's properties, relationships, and actions.
