...
The gap was avoidable. The design space for presenting structured information is narrower than the software industry assumes, and a few composable primitives — tiles, tile stacks, and master-detail views — cover it. The load-bearing primitive is a **recursive, orientation-flexible master-detail view generated as the default**: Miller columns that nest to arbitrary depth, let each level choose its own orientation, collapse automatically on narrow viewports, and are composed from model annotations rather than hand-built per screen. HCI research on mental models, information foraging, and Gestalt perception explains why a deliberately constrained UI grammar turns uniformity into an asset.

We present Strvct, an open-source JavaScript framework built on this idea. Centralizing the model-to-view pipeline yields capabilities that conventional component frameworks pay for per screen: automatic responsive design, headless testability, and annotation-driven persistence with cloud sync. It also makes the domain model natively operable by AI agents — the same annotations that generate the UI expose a constant-size tool surface that never drifts out of sync with the model — which, as agents become routine co-editors of application state, is the economic lever most likely to move naked objects past its twenty-five-year stall. A production application, undreamedof.ai, comprises ~90 domain classes with roughly 90% auto-generated views. Only two custom views remain, both for inherently graphical components: a 3D dice roller and a battle map.
...
1. The **recursive, orientation-flexible, generated master-detail view** as the primitive that closes the usability gap. Miller columns have navigated hierarchies since NeXTSTEP; our contribution is making them *recursive* (columns nest to arbitrary depth), *orientation-flexible* (each level chooses vertical or horizontal), *auto-collapsing* (the chain folds to a breadcrumb on narrow viewports), and the *generated default* (composed from model annotations, not hand-built per screen). This is the mechanism that turns "correct but form-like" generated UIs into ones with the spatial hierarchy and navigational depth users expect.
2. The *narrow design space* hypothesis: informational UIs converge on a small set of spatial conventions, making uniform application of those conventions an asset rather than a constraint — and making a small fixed grammar of primitives (tiles, tile stacks, and the recursive master-detail view above) sufficient to cover the space.
...
4. A **natively AI-operable domain model** as a demonstrated consequence of that annotation system: a single pair of tools covers every editable class, so the agent-facing tool surface is constant in the size of the model rather than growing linearly with it.
5. A working framework, Strvct, and a production application, undreamedof.ai, in which roughly 90% of views across ~90 domain classes are auto-generated.
