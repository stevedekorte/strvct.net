...
In practice, sub-pools correspond to the coarse-grained "documents" of the application: a character sheet, a campaign, a game session, an app's settings. Each is a self-contained graph that can be:
...
- **Access controlled** — permissions can be scoped to a sub-pool (e.g., a user owns their characters but has read access to a shared campaign)
