...
| Source | ARIA usage |
|---|---|
| `node.title()` | `aria-label` on tiles and views |
...
The label priority follows a natural mapping: `title()` provides the short instance-specific label that screen readers announce, while `subtitle()` and slot `description()` provide longer detail available on demand.
